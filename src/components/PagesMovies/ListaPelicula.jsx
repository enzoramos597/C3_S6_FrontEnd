...
import { API_MOVIES } from "../../services/api"
    ...
    useEffect(() => {
        const fetchPeliculas = async () => {
            ...
            try {
                ...
                const listaDePeliculas = res.data.peliculas || [];

                // Filtramos solo las películas activas
                const activas = listaDePeliculas.filter(
        ...
        <div className="min-h-screen bg-black/60 backdrop-blur-sm p-8 ">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 ">
                {currentMovies.map((peli) => {
                    ...
                    // 🟢 FUNCIÓN DE CLIC CONDICIONAL
                    const handleMovieClick = () => {
                        if (isAdmin) {
                            // Navega a la vista detallada administrativa (donde quizás hay más opciones)
                            navigate(`/admin/peliculas/${peli._id}`);
                        } else {
                            // Navega a la vista de reproducción/detalle del usuario estándar
                            navigate(`/user/peliculas/${peli._id}`);
                        }
                    };
                    ...
                    return (
                        ...
                        >
                            <div
                                onClick={handleMovieClick} // 👈 Usamos la función condicional
                            ...
                            {/* Botón solo Admin (Edición) */}
                            {isAdmin && (
                                <button
                                    onClick={(e) => {
                                        e.stopPropagation(); // 👈 IMPORTANTE: Evita que el clic se propague al div de arriba
                                        navigate(`/admin/updatemovie/${peli._id}`);
                                    }}
